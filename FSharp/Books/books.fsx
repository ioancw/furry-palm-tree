#r @"C:\Users\ioan_\Packages\packages\FSharp.Collections.ParallelSeq\lib\netstandard2.0\FSharp.Collections.ParallelSeq.dll"
#r @"C:\Users\ioan_\Packages\packages\MathNet.Numerics\lib\netstandard2.0\MathNet.Numerics.dll"
#r @"C:\Users\ioan_\Packages\packages\MathNet.Numerics.FSharp\lib\netstandard2.0\MathNet.Numerics.FSharp.dll"
...
///Whether the line is start or end of book.
...
let getBookTitle (lines: string []) =
    ...
    |> Array.pick (fun str ->
        if str.Contains "Title:" then
            Some
                (str.[6..].Trim()
                 |> String.map (fun letter -> if letter = ',' then ';' else letter))
        else
            None)

//Clean the text by skipping everything before book start
//and taking everyting until book end.
let cleanText (lines: string []) =
    lines
    |> Seq.skipWhile (bookStarted >> not)
    |> Seq.takeWhile (bookFinished >> not)
    |> String.concat " "
    |> Seq.map (fun c -> if Char.IsLetter c then Char.ToLowerInvariant c else ' ')
...
let bookBigrams filename =
    ...
    let rawBigrams =
        text |> Seq.windowed 2 |> Seq.countBy id |> dict
    let bigramCountVector =
        letterPairs
        |> Array.map (fun pair -> if rawBigrams.ContainsKey pair then rawBigrams.[pair] else 0)
...
//let filename =
//    @"/Users/ioanwilliams/github/furry-palm-tree/FSharp/Books/books/Dickens/pg46.txt"
...
let booksDir =
    @"C:\Users\ioan_\GitHub\furry-palm-tree\FSharp\Books\books"
...
// Ioan's run in FEB 2019: Real: 00:00:20.701, CPU: 00:01:39.956, GC gen0: 1569, gen1: 9
// Ioan's run PC Jun 2020: Real: 00:00:49.297, CPU: 00:02:44.062, GC gen0: 1299, gen1: 40, gen2: 1
// Save as csv file
let header =
    "Author,Book,"
    + (letterPairs
       |> Array.map (fun cs -> cs |> Array.map string |> String.concat "")
       |> String.concat ",")

let lines =
    Array.zip authorDirs authorBigrams
    |> Array.collect (fun (d, data) ->
...
let file =
    @"/Users/ioanwilliams/github/furry-palm-tree/FSharp/Books/books/bigramValues.csv"
...
#load "/Users/ioanwilliams//FSharp/packages/FSharp.Charting.0.90.9/FSharp.Charting.fsx"
...
let sortedBigrams =
    [| for idx in 0 .. letterPairs.Length - 1 ->
        let sum =
            fullData |> Array.sumBy (fun xs -> xs.[idx])
...
let mostUsedBigrams =
    sortedBigrams.[0..bigramCount - 1] |> set

let chooseBigrams bigramValues =
    bigramValues
    |> Array.mapi (fun i x -> if mostUsedBigrams.Contains i then Some(x) else None)
    |> Array.choose id
...
// This is covariance matrix, i.e. A * At (which you then do eigen value decomp on)
let covarianceMatrix =
    centeredMatrix * centeredMatrix.Transpose()

//You can do this manually or just use the EVD.
// Ax = b
// AtAx = At b
// AtA * AtA^-1 x = At * AtA^-1 * b
//You then find the inverse of AtA (via EVD)
...
let projection d =
    (eigenvectors d).Transpose() * centeredMatrix
...
titles
|> Array.iteri (fun i t -> printfn "%A %s" i t)
...
let kNearest k =
    [| for idx, v in projectedData.EnumerateColumnsIndexed() do
        if idx <> christmasCarolIdx then
            let distance =
                (v - christmasCarol) * (v - christmasCarol)

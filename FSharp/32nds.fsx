...
let parseFloat s = try float s with | _ -> 0. 
...
let tickToDecimalConverter (ticksString: string) = 
    ...
    let intPart, tick, eighth = 
        ...
        let eighth = tickPart.Substring(2, 1) |> function "+" -> 4. | other -> other |> parseFloat
        intPart, tick, eighth
...
let tickToDecimalConverter2 ticksString = 
    let treasuryStringRegex = Regex("^(?<int>\+|-?[0-9]+)-(?<tick>[0-9][0-9]?)(?<eighth>[0-9+]?)$", RegexOptions.Compiled)
    ...
    if matched.Success then 
        ...
        let eighth = matched.Groups.["eighth"].Value |> function "+" -> 4. | other -> parseFloat other 

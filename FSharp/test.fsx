...
let stringConcat (chars: char []) = System.String.Concat(chars)
let stringParse delimiter (s: string) = s.Split([| delimiter |])
let stringJoin delimiter (items: string []) = String.Join(delimiter, items)
...
let ngrams n (Token t) =
    t
    |> Seq.windowed (n + 1)
    |> Seq.map (Array.take n >> stringConcat >> Ngram)
    |> Seq.toArray
...
let ngramsToTokens (documents: Document []) =
    ...
    documents
    |> Array.collect (fun document -> document.Tokens) //array of Tokens (across all documents)
    |> Array.filter (fun (Token token) -> token.Length >= n) //only use Tokens that are greater than ngram size.
    |> Array.collect (fun token ->
        token
        |> getNgrams //Find the ngrams for everytoken, and return the tokens for a given ngram
...
let rec processTokens qs =
    ...
    | QTokens (ql, l) :: QOperator o :: QTokens (qr, r) :: t ->
        ...
        :: t
        |> processTokens
    | _ when (qs.Length % 2) = 0 -> failwith "Not a balanced query"
...
let searchFor queryFolder query =
    ...
    //taken the found document and checking for tokens (so we know the line number etc.)
    matchingFiles
...
let folderToIndex = @"C:\Users\ioan_\GitHub"

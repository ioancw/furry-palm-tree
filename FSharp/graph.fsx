...
type Meta<'t> when 't: comparison =
    ...
    override x.Equals y = compare x (y :?> Meta<_>) = 0
...
let search (graph: IGraph<_>) src dst =
    let rec loop (meta, closedSet, openSet) =
        ...
        | First(x, openSet) ->
            if x.vertex = dst then
                Some x.path, closedSet
            else
                ...
                let aux (meta, open_set) y =
                    if Set.contains y closedSet then
                        ...
                        let meta' openSet = Map.add y y' meta, Set.add y' openSet

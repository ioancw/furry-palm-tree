...
type Sack() =
    ...
    member t.AddBox(item) = boxes.Add(item)

    member t.Sum() = boxes |> Seq.sum
...
let packSleigh boxes volume =
    ...
    |> Array.iter
        (fun box ->
            ...
            | None ->
                let sack = Sack() //otherwise open sack
                sack.AddBox(box)
                sleigh.Add(sack))
...
let packSleigh2 boxes volume =
    ...
    |> Array.iter
        (fun box -> //work out if box is to be added to existing sack or new one
            let sackIndex =
                sleigh
...
let averageSimulation3 =
    Seq.fold (fun acc _ -> (packSleigh2 billybox vol) + acc) 0 { 1 .. vol }
    |> float

...
let removeDuplicates list = list |> Seq.distinct |> Seq.toList
...
let sortString (s: string) =
    s |> Seq.sort |> Seq.toList |> convertToString

let anagramKey (s: string) = sortString s

let isAnagram (s1: string) (s2: string) anagramKey = anagramKey s1 = anagramKey s2
...
let getAnagramsOf (dict: Dictionary<'key, ResizeArray<'T>>) word =
    ...
    if ok then anagrams else new ResizeArray<'T>()

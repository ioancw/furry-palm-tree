...
class BacktestLongOnly(BacktestBase):
    ...
    def plot_sma(self, SMA1, SMA2):
        plt.style.use('ggplot')
        title = 'Price and SMAs: %s | SMA1 = %d, SMA2 = %d' % (self.symbol, SMA1, SMA2)
        self.data[['price', 'SMA1', 'SMA2']].plot(title=title, figsize=(10, 6))
    ...
    def run_sma_strategy(self, SMA1, SMA2):
        ...
        self.plot_sma(SMA1, SMA2)
   
        for bar in range(len(self.data)):
            if bar >= SMA2:
                if self.position == 0:
                    if self.data['SMA1'].ix[bar] > self.data['SMA2'].ix[bar]:
                        print('SMA1',self.data['SMA1'].ix[bar])
                        print('SMA2', self.data['SMA2'].ix[bar])
...
if __name__ == '__main__':
    ...
    #lobt = BacktestLongOnly('RBS', '2010-1-1', '2016-10-31', 10000)
    #lobt.verbose = True
    #run_strategies()
    lobt = BacktestLongOnly('DIS', '2010-1-1', '2016-10-31',
                            10000, 10.0, 0.000)  # 10 USD fix, 0.5% variable
    lobt.verbose = True
    run_strategies()